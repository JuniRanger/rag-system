...
* **Entradas (placeholders):**
  ...
  * `{conversation_history}` — historial reciente (si el plan lo incluye)
...
* **Dependencias:** `WorkingMemory.to_prompt_text()`, `_build_context()`, `plan.conversation_history`.
...
* **Entradas:** `{working_memory}`, `{conversation_history}`, `{context}`, `{question}`.
